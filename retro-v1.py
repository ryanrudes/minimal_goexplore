...
def batch_step(actions, repeat, render=False):
  ...
  for action, env in zip(actions, envs):
    for i in range(repeat):
      if solo_render and frame % frameskip == 0: envs[0].render()
      state, reward, terminal, info = env.step(np.eye(num_actions)[action])
    states.append(state)
...
# Cell parameters
downsampled_height, downsampled_width = (7, 7)
downsampled_depth = 8

# Exploration parameters
sticky_actions_repeat = 32
...
env_name = input("Enter Environment Name ID (ie. SuperMarioWorld2-Snes). Make sure you've properly loaded in the ROM file for the game: ")
...
archive.add(initial_reference, score, False, None, empty_trajectory, envs[0].em.get_state(), initial_cell, True)
...
while True:
  ...
  rams = [env.em.get_state() for env in envs]
  ...
  if any(terminals):
    ...
    for i, terminal in enumerate(terminals):
      if terminal:
        ...
        envs[i].reset()
        envs[i].em.set_state(archive.get(ref).get('ram'))
